...
// Import the GLB as a Vite asset — Vite resolves the correct URL for dev + production
// @ts-ignore – Vite handles ?url imports at build time
import petModelUrl from '/pet-model.glb?url';

// ─────────────────────────────────────────────────────────────
// === GLB Model Loading (primary) ===
// ─────────────────────────────────────────────────────────────

// Shared DRACOLoader instance (decoder binaries from Google CDN)
const dracoLoader = new DRACOLoader();
dracoLoader.setDecoderPath('https://www.gstatic.com/draco/versioned/decoders/1.5.7/');
dracoLoader.preload();

/**
 * Fix material textures to ensure they render with correct color encoding.
 * GLB models use PBR materials that need SRGB textures to display properly.
 */
function fixMaterial(mat: THREE.Material): void {
...
/**
 * Load the GLB pet model asynchronously and add it to a parent group.
 * Falls back to the procedural pet if loading fails.
 *
 * @param parent - The THREE.Group to add the loaded model to
 */
export function loadPetModel(parent: THREE.Group): void {
  const loader = new GLTFLoader();
  loader.setDRACOLoader(dracoLoader);

  // Resolve URL: use Vite import if available, fallback to public path
  const modelUrl = petModelUrl || '/pet-model.glb';
  console.log('[PetGeometry] Loading model from:', modelUrl);
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      // Ensure all materials and textures render correctly
      model.traverse((child) => {
        if ((child as THREE.Mesh).isMesh) {
          const mesh = child as THREE.Mesh;
          // Ensure materials respond to lighting
          if (Array.isArray(mesh.material)) {
            mesh.material.forEach((mat) => fixMaterial(mat));
      ...
      // Auto-scale the model to fit roughly the same bounds as the procedural pet
      const box = new THREE.Box3().setFromObject(model);
      const size = new THREE.Vector3();
      box.getSize(size);
      const maxDim = Math.max(size.x, size.y, size.z);
      const targetSize = 3.9; // make the model nice and prominent
      ...
      model.position.y += 0.46; // lift to avoid bottom clipping

      model.name = 'glb-pet';

      // Remove procedural fallback if it was added
      const fallback = parent.getObjectByName('procedural-pet');
      if (fallback) parent.remove(fallback);

      parent.add(model);
      console.log('[PetGeometry] GLB model loaded successfully');
    },
    (progress) => {
      if (progress.total > 0) {
        const pct = ((progress.loaded / progress.total) * 100).toFixed(0);
        console.log(`[PetGeometry] Loading model: ${pct}%`);
      }
    },
    (error) => {
      console.warn('[PetGeometry] Failed to load GLB model, using procedural fallback:', error);
      // Fallback is already added by createPet(), so nothing extra needed
    },
  );
}

/**
 * Creates the pet - loads GLB model with procedural fallback
 * The procedural pet is shown immediately while the GLB loads.
 * Once the GLB loads, it replaces the procedural version.
 */
export function createPet(): THREE.Group {
  const pet = new THREE.Group();

  // Add procedural pet as immediate fallback (shown while GLB loads)
  const proceduralPet = createProceduralPet();
  proceduralPet.name = 'procedural-pet';
  pet.add(proceduralPet);

  // Start loading the GLB model (will replace procedural pet when ready)
  loadPetModel(pet);
...
// ─────────────────────────────────────────────────────────────
// === Procedural Pet (backup / fallback) ===
// ─────────────────────────────────────────────────────────────
...
/**
 * Creates the original procedural 3D cartoon pet from Three.js primitives.
 * Kept as fallback in case the GLB model fails to load.
 *
 ...
 */
function createProceduralPet(): THREE.Group {
  const pet = new THREE.Group();
